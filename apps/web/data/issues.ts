...
export const issues: Issue[] = [
  ...
  },
  {
    ...
    reasoning: "Agent tabbed through the entire dashboard in DOM order. The logout button exists in the DOM inside a user-menu dropdown but is hidden via display:none when the menu is closed. While display:none correctly removes it from the tab order, the only way to open the dropdown is by clicking the user avatar — there is no keyboard event listener on the avatar element, no aria-haspopup attribute, and no keyboard shortcut documented. Keyboard-only users have no path to log out.",
    severity: "error", status: "open", element: "button#logout",
  },
  {
    id: "iss-15", runId: "run-btn-1", nodeId: "dashboard", category: "other",
    ...
    reasoning: "Agent deselected all table rows and clicked the Export button. The button appeared interactive (cursor:pointer, normal opacity) and accepted the click event without any visual change, toast notification, or error message. No network request was initiated. Users have no way to know the action is unavailable. The button should either be visually disabled (disabled attribute + aria-disabled) with a tooltip explaining why, or show an immediate error toast explaining that a row selection is required.",
    severity: "warning", status: "open", element: "button#export-data",
  },
]
