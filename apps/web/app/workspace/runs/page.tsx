...
export default function RunsPage() {
  const totalFailed   = runSteps.filter((s) => s.status === "failed").length
  const totalWarnings = runSteps.filter((s) => s.status === "warning").length
  const totalPassed   = runSteps.filter((s) => s.status === "passed").length
      ...
      <div className="flex min-w-0 flex-1 flex-col overflow-y-auto">
        ...
        <div className="border-b border-[#c7d9f0] bg-white">
          <div className="flex items-stretch divide-x divide-[#c7d9f0]">
            {([
              { label: "TOTAL STEPS", value: runSteps.length, color: "text-[#1a2a33]" },
